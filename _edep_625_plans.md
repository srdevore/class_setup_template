...
## Context
You're teaching a new SEM class in the fall, structurally similar to EDEP 606. Current pain point: emailing rlab files to students each week is tedious. Students don't know GitHub and you won't cover it. You want:
- A fresh repo (EDEP 606 stays as the archive)
- Syllabus, slides, assignments, rlabs all in GitHub (for you)
- Easy, no-install access for students (for them)
- A smoother rlab distribution than email
...
|-------|------|-------------|
| Source of truth | **GitHub repo** (Quarto .qmd files) | You, locally in RStudio |
| Public website | **GitHub Pages** auto-built from Quarto | Students bookmark this URL |
| Runnable R labs | **Posit Cloud Space** | Students click → code runs in browser |
| Submissions + grading | **Lamaku** (Brightspace/D2L) | Students upload work; you grade |

Students never touch GitHub. They get two URLs at the start of the semester: the class website and the Posit Cloud space invite link.

## Why this combo

- **Quarto + GitHub Pages**: you push, the site rebuilds, students get the update. Slides become revealjs HTML decks (browser-viewable, no PowerPoint needed). Assignments render to HTML/PDF. Syllabus is just another page.
- **Posit Cloud** is the right answer to "how do I share rlabs without emailing them." You set up one *Space*, drop new lab projects into it each week, and students see them appear under one persistent link. They click "Save a permanent copy" → they have their own runnable version in seconds, no install, no version confusion, no attachments.

## Proposed repo structure

```
edep625-fall26/                   ← new GitHub repo, public
  _quarto.yml                     ← site config (navbar, theme)
  index.qmd                       ← homepage / weekly schedule
  syllabus.qmd                    ← syllabus as a webpage
  lectures/
    w01-intro-to-sem.qmd          ← renders to revealjs HTML deck
    w02-path-analysis.qmd
    ...
  rlabs/
    w01-lavaan-basics.qmd         ← mirrored into Posit Cloud
    w02-fit-indices.qmd
    ...
  assignments/
    hw01-path-models.qmd
    take-home-1.qmd
    ...
    research-connection.qmd   ← presentation instructions, rubric, sign-up schedule
    final-paper.qmd           ← paper guidelines, rubric, milestone due dates
  data/
    *.csv
  .github/workflows/publish.yml   ← auto-deploys site on push
  README.md
```
...
```bash
gh repo create edep625-fall26 --public --clone
```
Public so GitHub Pages is free; private repos require a paid plan for Pages.

### 2. Scaffold a Quarto website
- Confirm Quarto is installed: `quarto check`
- `quarto create project website .` inside the repo
- Edit `_quarto.yml` to add a navbar with: Home, Syllabus, Lectures, Labs, Assignments (with a dropdown for: Weekly homework, Research connection, Final paper)
- Pick a theme (e.g., `theme: cosmo`) — looks clean out of the box

### 3. Slide decks via revealjs
Each lecture `.qmd` starts with:
```yaml
---
title: "Week 1: Intro to SEM"
format:
  revealjs:
    theme: simple
    slide-number: true
---
```
`quarto render` produces a self-contained HTML deck — arrow keys to navigate, embed R code chunks that execute at render time. If you really want a `.pptx` alongside, add `pptx` to the format list and Quarto produces both.

### 4. GitHub Pages auto-deploy
Add `.github/workflows/publish.yml` using Quarto's official action:
```yaml
on:
  push:
    branches: [main]
...
```
In repo Settings → Pages → set source to `gh-pages` branch. Every `git push` to main = updated site at `<username>.github.io/edep625-fall26/`.
...
## Teacher vs student slide versions (Quarto profiles)

One `.qmd` per lecture, two output versions — your teacher copy with notes/answers, the student copy without. Wrap conditional content in profile-aware blocks:

````markdown
## Path analysis example

Here's the model we'll fit:

::: {.content-visible when-profile="teacher"}
**Answer key:** the chi-square will be ~14 because the model has 3 df…
**Pacing note:** spend extra time on the df calculation here
:::

::: {.content-hidden when-profile="teacher"}
Try fitting this model yourself before class.
:::
````

Two renders, same source file:
- `quarto render --profile teacher` → your version with notes/answers (run locally for your own use)
- `quarto render` (default = student) → version that goes to the website

The GitHub Action only runs the default render, so **only the student version is ever published**. Your teacher version stays local — never reaches the public site.

For genuinely separate documents (full answer keys, solution sets), use the underscore-prefix convention: anything starting with `_` (file or folder) is skipped by Quarto's site build but stays in the repo for your reference:
```
_teacher-notes/
  hw01-full-solutions.qmd   ← in repo, never published
```

Combined pattern: profiles for slides (one source, less drift), `_teacher-notes/` for whole separate documents like full answer keys.
